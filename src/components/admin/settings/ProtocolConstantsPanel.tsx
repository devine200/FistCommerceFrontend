...
const ProtocolConstantsPanel = () => {
  return (
    <SettingsPanel
      ...
      description="Hardcoded contract constants with no on-chain setter. Configurable values live in the panels above."
      badge={{ label: 'Not configurable', variant: 'neutral' }}
    >
      {DEFAULT_PROTOCOL_CONSTANTS.length === 0 ? (
        <p className="text-[#6B7488] text-[14px] rounded-[8px] border border-dashed border-[#D1D5DB] bg-[#FAFBFC] px-5 py-8 text-center">
          No immutable constants to display. Banker year, withdrawal timing, and min repayment are
          configurable via Risk &amp; Allocation, Funding Pool, and Payout Router panels.
        </p>
      ) : (
