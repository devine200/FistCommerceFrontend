...
const InvestorBalanceSummaryBoxes = ({
  ...
  const chainId = useAppSelector((s) => s.auth.chainId ?? s.wallet.chainId)
  const tokenName = getAcceptedTokenDisplayName(chainId)
  return (
    <div className={`grid grid-cols-1 sm:grid-cols-2 gap-3 ${className ?? ''}`}>
      <div className="rounded-[6px] border border-[#E6E8EC] bg-[#F9FAFB] px-4 py-3 text-left">
        ...
        <p className="mt-1 text-[#8B92A3] text-[11px]">{tokenName} token balance</p>
      </div>
