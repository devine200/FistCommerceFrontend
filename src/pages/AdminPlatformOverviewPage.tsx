...
const AdminPlatformOverviewPage = () => {
  ...
  const { metricCards, activities, adminMetrics, tvlHistory, originatedHistory, status, lastUpdated } =
    useAppSelector((s) => s.adminDashboard)

  const tvlSeries = useMemo(
    () => adminChartHistoryToRechartsData(tvlHistory, 'tvl'),
    [tvlHistory],
  )
  const originatedSeries = useMemo(
    () => adminChartHistoryToRechartsData(originatedHistory, 'principal'),
    [originatedHistory],
  )
  ...
  return (
    <AdminPageFrame>
      ...
      <AdminOverviewCharts
        ...
        tvlSeries={tvlSeries}
        originatedSeries={originatedSeries}
      />

      <AdminPanel>
        <div className="px-5 py-4 border-b border-[#EDF0F4]">
          <h2 className="text-[#0B1220] font-bold text-[18px]">Recent repayments</h2>
          <p className="text-[#6B7488] text-[13px] mt-1">Indexed on-chain repayment events</p>
        </div>
        {activities.length === 0 ? (
          <p className="px-5 py-8 text-[#6B7488] text-[14px]">
            {status === 'loading'
              ? 'Loading recent repayments…'
              : 'No repayment events indexed yet. Sync dashboard data after on-chain repayments are indexed.'}
          </p>
        ) : (
          <ul className="divide-y divide-[#EDF0F4]">
            {activities.map((row) => (
              <li key={`${row.title}-${row.subtitle}-${row.date}`}>
                <button
                  type="button"
