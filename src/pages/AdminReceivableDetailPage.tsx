...
const AdminReceivableDetailPage = () => {
  ...
  useEffect(() => {
    ...
    void fetchAdminReceivableDetail(accessToken, receivableId)
      ...
      .catch((e) => {
        if (cancelled) return
        setError(e instanceof Error ? e.message : 'Could not load receivable detail.')
        setStatus('failed')
      })
    return () => {
      cancelled = true
  ...
  if (status === 'loading') {
    return (
      <AdminPageFrame>
        <p className="text-[#6B7488] text-[14px]">Loading receivable details…</p>
      </AdminPageFrame>
    )
  }
  ...
  if (status === 'failed' || !detail) {
    return (
      <AdminPageFrame>
        <p className="text-[#DC2626] text-[14px]">{error ?? 'Receivable not found.'}</p>
        <button
          type="button"
          onClick={() => navigate('/dashboard/admin/receivables')}
          className="mt-4 text-[#195EBC] text-[14px] font-semibold hover:underline"
        >
          Back to receivables
        </button>
      </AdminPageFrame>
